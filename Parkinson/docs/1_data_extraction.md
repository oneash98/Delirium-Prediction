...
- LOS/24시간/ICU 입실 24시간 이후 Delirium assessment 존재 기준의 inclusion/exclusion criteria 적용
- 문자열 value의 숫자화
- 온도, 체중, 키, FiO2 등 단위 변환
- multi-unit feature 분리
- 12시간 구간 라벨링 및 charttime 기준 wide table 생성
...
- 이 노트북은 extraction만 수행합니다. 12시간 라벨 long-format event, charttime 기준 wide table, assessment index 생성은 `2_data_transform.ipynb`, EDA는 `3_eda.ipynb`, train/test split과 모델링 입력 준비는 `4_modeling.ipynb`에서 처리합니다.
