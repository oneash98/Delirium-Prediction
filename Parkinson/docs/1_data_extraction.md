...
- ICU 입실 시점 이전 가장 최근 `services.curr_service`에서 온 `specialty`
- `intime`, `outtime`
- `icu_los_hours`
...
- `patients` 항목인 age/gender는 별도 event table로 추출하지 않고 cohort table인 `adm_pat_icu.csv`에 포함됩니다.
...
- 일반 chart event는 `charttime`이 ICU `intime`과 `outtime` 사이에 있는 행만 유지합니다.
- `height`, `weight`는 static body measurement로 취급합니다. MIMIC에서 ICU 입실 전 병원 입원 중에 기록되는 경우가 많아, hospital admission 안의 첫 측정값을 stay별 `intime` 시점 row로 붙입니다. `height`는 해당 admission에 값이 없으면 subject의 첫 height 기록을 fallback으로 사용합니다.
...
- Chemistry/electrolytes: glucose, bicarbonate, chloride, sodium, potassium, BUN, creatinine, magnesium, phosphate
- Calcium/bilirubin 계열: calcium, total/direct bilirubin
- CBC: hematocrit, hemoglobin, platelet count, WBC
- Liver/nutrition: ALT, AST, albumin
- ABG/coagulation/endocrine/metabolic: lactate, pH, pCO2, pO2, INR, PT, PTT, ammonia, cortisol
...
- 이 노트북은 extraction만 수행합니다. 12시간 라벨 long-format event, charttime 기준 wide table, 12시간 bin-level wide table, assessment index 생성은 `2_data_transform.ipynb`, EDA는 `3_eda.ipynb`, train/test split과 LSTM sequence index 생성은 `4_train_test_construction.ipynb`에서 처리합니다.
