...
`hours`는 현재 bin까지의 ICU 경과시간으로 input feature에 포함합니다. `race`는 categorical feature로 포함합니다. `los_hours`는 전체 ICU 재원시간이라 미래 정보 성격이 있으므로 preprocessing에서 제외합니다. `admission_type`, `specialty`도 현재 모델 입력에서 제외합니다.
...
- 12시간 bin 안에 exposure가 있으면 `1`, 아니면 `0`
...
Medication feature는 transform 단계에서 12시간 bin-level exposure로 생성됩니다. Preprocessing 단계에서는 catalog 기준 binary feature로 분류하고 0/1 그대로 사용합니다.

Procedure/device feature는 transform 단계에서 procedure interval이 12시간 bin과 겹치면 `1`입니다. Preprocessing 단계에서는 catalog 기준 binary feature로 분류하고 0/1 그대로 사용합니다.
...
각 연속형 변수는 transform 단계의 12시간 bin 집계값을 사용합니다. `most recent` 변수는 bin 안의 최신값을 사용하고 같은 stay 안에서 forward fill합니다. 첫 번째 bin이 비어 있고 두 번째 bin에 값이 있으면 첫 번째 bin만 두 번째 bin의 최신값으로 채웁니다. `aggregation` 변수는 bin 안의 요약값을 쓰며, 측정 횟수 `count < 3`이면 `std`를 `NaN`으로 둡니다. 빈 bin은 직전 `latest` 하나로 `mean`, `median`, `min`, `max`, `latest`를 채우고 `count = 1`, `std = NaN`으로 둡니다. 첫 번째 bin이 비어 있으면 두 번째 bin의 가장 이른 측정값으로 같은 방식으로 채웁니다.
...
- numeric binary feature: 0 imputation, scaling 없음
