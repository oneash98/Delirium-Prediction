...
- 입력 길이: 4
- 출력 길이: 2 (`t+1`, `t+2`)
- anchor 후보: `t1`부터 미래 label이 하나 이상 있는 bin
...
| --- | --- | --- | --- |
| `t1` | `PAD,PAD,PAD,t1` | `t2,t3` | 실제 label이 있는 target |
| `t2` | `PAD,PAD,t1,t2` | `t3,t4` | 실제 label이 있는 target |
| `t3` | `PAD,t1,t2,t3` | `t4,t5` | 실제 label이 있는 target |
| `t4` | `t1,t2,t3,t4` | `t5,t6` | 실제 label이 있는 target |
| 마지막 직전 bin | 최근 4개 bin | `t_last,PAD` | `t_last` |

예를 들어 bin이 10개인 stay는 anchor `t1`부터 `t9`까지 최대 9개 sequence row를 만들 수 있습니다. PAD 3개와 `t1`만 있는 sequence를 만들며, 이 row가 `0-12hr` 정보로 `12-24hr` label을 예측하는 첫 example입니다.
